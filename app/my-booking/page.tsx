...
const statusConfig: Record<string, { label: string; color: string; bg: string; border: string; icon: any; gradient: string }> = {
  pending_otp: { 
    ...
    icon: Clock3,
    gradient: 'from-amber/5 to-amber/[0.02]'
  },
  confirmed: { 
    label: 'Confirmed', 
    ...
    icon: CheckCircle2,
    gradient: 'from-sage/5 to-sage/[0.02]'
  },
  cancelled: { 
    label: 'Cancelled', 
    color: 'text-red-600', 
  ...
  completed: { 
    ...
    icon: CheckCircle2,
    gradient: 'from-ink-muted/5 to-ink-muted/[0.02]'
  },
  no_show: { 
    label: 'No Show', 
    color: 'text-dusty', 
    ...
    icon: Clock3,
    gradient: 'from-dusty/5 to-dusty/[0.02]'
  },
};

const paymentConfig: Record<string, { label: string; color: string; bg: string }> = {
  pending: { label: 'Pending', color: 'text-amber-dark', bg: 'bg-amber/10' },
  paid: { label: 'Paid', color: 'text-sage-dark', bg: 'bg-sage/10' },
  failed: { label: 'Failed', color: 'text-red-600', bg: 'bg-red-50' },
  refunded: { label: 'Refunded', color: 'text-dusty', bg: 'bg-dusty/10' },
};
            ...
            >
              <div className="relative">
                ...
                <form onSubmit={handleSubmit} className="relative">
                  <div className="flex flex-col sm:flex-row gap-3 p-2 sm:p-2.5 rounded-2xl sm:rounded-3xl bg-white/90 backdrop-blur-xl border border-white/60 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.1)]">
                    <div className="relative flex-1">
                      <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-ink-light/50" />
                      <input
                        type="text"
                        value={bookingId}
                        onChange={(e) => setBookingId(e.target.value)}
                        placeholder="Enter booking ID (e.g. CH-XXXX-XXXX)"
                        className="w-full pl-12 pr-4 py-3.5 sm:py-4 text-sm sm:text-base bg-transparent border-0 outline-none placeholder:text-ink-light/40 text-ink font-medium"
                      />
                    </div>
                    <motion.button
                      type="submit"
              ...
              {!booking && !error && !loading && (
                <motion.div 
                  ...
                  className="mt-4 flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-xs text-ink-light/60"
                >
              ...
              >
                ...
                <div className={`relative px-5 sm:px-8 pt-6 sm:pt-8 pb-6 bg-gradient-to-br ${statusConfig[booking.bookingStatus]?.gradient || 'from-ink-muted/5 to-ink-muted/[0.02]'} border-b border-surface-border/40`}>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={handleCloseModal}
                    className="absolute top-4 right-4 sm:top-6 sm:right-6 p-2 rounded-xl bg-white/80 border border-surface-border/40 text-ink-light hover:text-burgundy hover:border-burgundy/20 transition-colors z-10"
                  >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pr-10 sm:pr-0">
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className={`flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl ${statusConfig[booking.bookingStatus]?.bg || 'bg-ink-muted/10'} border ${statusConfig[booking.bookingStatus]?.border || 'border-ink-muted/20'} shrink-0`}>
                        {(() => {
                          ...
                          return <StatusIcon className={`h-6 w-6 sm:h-7 sm:w-7 ${statusConfig[booking.bookingStatus]?.color || 'text-ink-muted'}`} />;
                        })()}
                      </div>
                      <div>
                        <h2 className={`text-xl sm:text-2xl font-bold ${statusConfig[booking.bookingStatus]?.color || 'text-ink-muted'}`}>
                          {statusConfig[booking.bookingStatus]?.label || booking.bookingStatus}
                        </h2>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${paymentConfig[booking.paymentStatus]?.bg || 'bg-ink-muted/10'} ${paymentConfig[booking.paymentStatus]?.color || 'text-ink-muted'} border-current/20`}>
                            {paymentConfig[booking.paymentStatus]?.label || booking.paymentStatus}
                          </span>
                          <span className="text-xs text-ink-light">•</span>
                          <span className="text-xs text-ink-light">Booked {formatShortDate(booking.createdAt)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs sm:text-sm text-ink-muted bg-white/80 px-3 sm:px-4 py-2 rounded-xl border border-surface-border/60 font-medium tracking-wider">
                        {booking.bookingId}
                      </span>
                      <motion.button
                        ...
                        className="p-2 rounded-xl bg-white/80 border border-surface-border/60 text-ink-light hover:text-burgundy hover:border-burgundy/20 transition-colors"
                  ...
                  <div className="grid gap-6 sm:gap-8">
                    ...
                    >
                      <div className="flex items-center gap-2.5 mb-4">
                        ...
                        <h3 className="font-accent text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Schedule</h3>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="flex items-center gap-3.5 p-4 rounded-2xl bg-cream-warm/50 border border-white/60">
                          ...
                          <div className="min-w-0">
                            <p className="text-[10px] sm:text-[11px] text-ink-muted font-accent uppercase tracking-[0.15em] font-semibold">Date</p>
                        ...
                        <div className="flex items-center gap-3.5 p-4 rounded-2xl bg-cream-warm/50 border border-white/60">
                          ...
                          <div>
                            <p className="text-[10px] sm:text-[11px] text-ink-muted font-accent uppercase tracking-[0.15em] font-semibold">Time</p>
                    ...
                    >
                      <div className="flex items-center gap-2.5 mb-4">
                        <div className="h-6 w-1 rounded-full bg-burgundy/40" />
                        <h3 className="font-accent text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Venue</h3>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="flex items-center gap-3.5 p-4 rounded-2xl bg-cream-warm/50 border border-white/60">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-burgundy/[0.08] text-burgundy shrink-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-[10px] sm:text-[11px] text-ink-muted font-accent uppercase tracking-[0.15em] font-semibold">Location</p>
                            <p className="text-xs sm:text-sm font-bold text-ink truncate">
                              {booking.location?.name || 'N/A'}
                              {booking.location?.city ? <span className="text-ink-light font-normal">, {booking.location.city}</span> : ''}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3.5 p-4 rounded-2xl bg-cream-warm/50 border border-white/60">
                          ...
                          <div className="min-w-0">
                            <p className="text-[10px] sm:text-[11px] text-ink-muted font-accent uppercase tracking-[0.15em] font-semibold">Room</p>
                    ...
                    >
                      <div className="flex items-center gap-2.5 mb-4">
                        <div className="h-6 w-1 rounded-full bg-burgundy/40" />
                        <h3 className="font-accent text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Package Details</h3>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="flex items-center gap-3.5 p-4 rounded-2xl bg-cream-warm/50 border border-white/60">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-burgundy/[0.08] text-burgundy shrink-0">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-[10px] sm:text-[11px] text-ink-muted font-accent uppercase tracking-[0.15em] font-semibold">Package</p>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-ink truncate">{booking.package?.name || 'N/A'}</span>
                              {booking.package?.tier && (
                                <span className="text-[9px] sm:text-[10px] uppercase tracking-wider font-accent font-bold px-2 py-0.5 rounded-full bg-amber/10 text-amber-dark border border-amber/20">
                                  {booking.package.tier}
                                </span>
                              )}
                            </div>
                        ...
                        <div className="flex items-center gap-3.5 p-4 rounded-2xl bg-cream-warm/50 border border-white/60">
                          ...
                          <div>
                            <p className="text-[10px] sm:text-[11px] text-ink-muted font-accent uppercase tracking-[0.15em] font-semibold">Guests</p>
                            <p className="text-xs sm:text-sm font-bold text-ink">
                              {booking.guests} <span className="text-ink-light font-normal">{booking.guests === 1 ? 'person' : 'people'}</span>
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    {booking.addOns && booking.addOns.length > 0 && (
                      ...
                      >
                        <div className="flex items-center gap-2.5 mb-4">
                          ...
                          <h3 className="font-accent text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Add-Ons</h3>
                        </div>
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  ...
                                  {item.quantity > 1 && (
                                    <p className="text-[10px] sm:text-xs text-ink-muted">Qty: {item.quantity}</p>
                                  )}
                                </div>
                              </div>
                              {item.addon?.price && (
                    ...
                    {booking.specialRequests && (
                      ...
                      >
                        <div className="flex items-center gap-2.5 mb-4">
                          ...
                          <h3 className="font-accent text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Special Requests</h3>
                        </div>
                    ...
                    >
                      ...
                      <div className="p-4 sm:p-5 rounded-2xl bg-white/60 border border-white/70">
                        <div className="flex items-center gap-2.5 mb-4">
                          ...
                          <h3 className="font-accent text-[10px] sm:text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Customer</h3>
                        </div>
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] sm:text-xs text-ink-light">Primary contact</p>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <div className="flex items-center gap-2.5 text-xs sm:text-sm text-ink-secondary">
                            <Mail className="h-3.5 w-3.5 text-ink-light shrink-0" />
                            <span className="truncate">{booking.customerDetails?.email}</span>
                          </div>
                          <div className="flex items-center gap-2.5 text-xs sm:text-sm text-ink-secondary">
                            <Smartphone className="h-3.5 w-3.5 text-ink-light shrink-0" />
                      ...
                      <div className="p-4 sm:p-5 rounded-2xl bg-white/60 border border-white/70">
                        <div className="flex items-center gap-2.5 mb-4">
                          <div className="h-5 w-1 rounded-full bg-burgundy/30" />
                          <h3 className="font-accent text-[10px] sm:text-xs uppercase tracking-[0.2em] text-ink-muted font-semibold">Payment</h3>
                        </div>
                        <div className="space-y-2.5">
                ...
                <div className="px-5 sm:px-8 py-4 sm:py-5 border-t border-surface-border/30 bg-white/40 backdrop-blur-sm flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-2 text-[10px] sm:text-xs text-ink-light/60 justify-center sm:justify-start">
                    <ShieldCheck className="h-3.5 w-3.5" />
